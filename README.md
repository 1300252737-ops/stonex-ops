...
The daily report is generated by the Claude `daily-report` skill and can be
pushed to Discord.
...
```bash
cp .env.example .env
# Edit .env and set your DISCORD_URL
```

### Push a report

```bash
echo "report content here" | scripts/push-discord.sh
```

### Cron (auto-push at 08:00)

If you want Claude to push automatically every morning, keep Claude Code
running and use `/loop` or CronCreate inside the session.

For a Claude-independent setup, run the report script via system cron.
Example crontab entry (run daily at 08:00):

```cron
0 8 * * * cd /path/to/stonex-ops && echo "Report" | scripts/push-discord.sh
```
